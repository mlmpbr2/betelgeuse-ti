...
# REDIRECT URI — CRÍTICO: deve bater EXATAMENTE com o configurado no Facebook Developer
# Hugging Face Spaces usa HTTPS sempre. A URL é: https://<space-name>.hf.space/login/callback
...
logger.info(f"REDIRECT_URI configured: {REDIRECT_URI}")
...
.stat-card{background:linear-gradient(135deg,#1877f2 0%,#166fe5 100%);color:#fff;padding:22px;border-radius:16px;text-align:center}
.stat-card.green{background:linear-gradient(135deg,#42b72a 0%,#36a420 100%)}
...
.debug-box{background:#1a1a2e;color:#00ff88;padding:16px;border-radius:8px;font-family:monospace;font-size:12px;margin:16px 0;overflow-x:auto}
...
<div class="card">
  ...
  <p class="muted">This tool enables social media teams to monitor and moderate public comments on Facebook Pages they manage. <strong>No personal data is stored on our servers.</strong> All processing is done in real-time during the active session.</p>
...
<div class="card login-hero">
  ...
  <p>Sign in with the Facebook account that administers your Page.</p>

  <div class="perm-grid" style="max-width:600px;margin:0 auto 28px;text-align:left">
    <div class="perm-item">
      <div class="perm-icon std">📄</div>
      <div><div class="perm-title">pages_show_list — List your Pages</div><div class="perm-desc">Shows Pages you administer so you can choose which one to monitor.</div></div>
    </div>
    <div class="perm-item">
      <div class="perm-icon adv">📊</div>
      <div><div class="perm-title">pages_read_engagement — Read posts & metrics</div><div class="perm-desc">Reads posts from the selected Page to let you choose which post to moderate.</div></div>
    </div>
    <div class="perm-item">
      <div class="perm-icon sens">💬</div>
      <div><div class="perm-title">pages_read_user_content — Read comments</div><div class="perm-desc">Reads comment author name, message, and date for real-time moderation. Data is not stored.</div></div>
    </div>
  </div>
  ...
  <div class="info-box" style="max-width:500px;margin:24px auto 0;text-align:left">
    <h4 style="margin-top:0">🔒 Privacy Commitment</h4>
    <p class="muted">• No data stored on our servers — real-time processing only</p>
    <p class="muted">• Session data cleared on logout</p>
    <p class="muted">• You can revoke access anytime in Facebook Settings</p>
    <p class="muted">• CNPJ-verified business (51.770.524/0001-87)</p>
  </div>
</div>
...
<div class="alert alert-success">
  ...
  <div><strong>Authenticated successfully.</strong> Select a Page below. <a href="/logout" style="color:#2e7d32;font-weight:700">Disconnect</a></div>
</div>
...
<div class="card">
  ...
  <div class="stats-grid">
    <div class="stat-card"><div class="stat-num">{{comments|length}}</div><div class="stat-label">Total Comments</div></div>
    <div class="stat-card green"><div class="stat-num">{{total_likes}}</div><div class="stat-label">Total Likes</div></div>
    <div class="stat-card orange"><div class="stat-num">{{negative_count}}</div><div class="stat-label">Flagged for Review</div></div>
  </div>
  ...
  <div style="margin-top:16px">
    ...
    <div class="comment-card {% if c.is_negative %}negative{% endif %}">
      <div class="comment-author">👤 {{c.author_name}} <span class="comment-id">ID: {{c.author_id}}</span>{% if c.is_negative %}<span class="badge badge-sens" style="font-size:11px;padding:2px 8px">FLAGGED</span>{% endif %}</div>
      <div class="comment-text">{{c.message}}</div>
      <div class="comment-meta"><span>📅 {{c.created_time}}</span><span>❤️ {{c.like_count}} likes</span></div>
    </div>
    {% endfor %}
  </div>
  {% else %}
  ...
  {% endif %}
  <div class="divider"></div>
  <div style="text-align:center"><a href="/?page={{sel|urlencode}}" class="btn btn-outline" style="font-size:13px">← Back to Posts</a></div>
</div>
{% endif %}
...
def get_posts(page_id, page_token):
    try:
        ...
        data = r.json()
        if "error" in data:
            logger.error(f"posts API error: {data['error']}")
            return [], data["error"]
        posts = data.get("data", [])
        for p in posts:
        ...
        logger.error(f"posts exception: {e}")
        return [], str(e)
...
def get_comments(post_id, page_token):
    try:
        ...
        data = r.json()
        if "error" in data:
            logger.error(f"comments API error: {data['error']}")
            return [], data["error"]
        raw = data.get("data", [])
        ...
        for c in raw:
            ...
            processed.append({
                "id": c.get("id"), "author_name": author.get("name", "Unknown"),
                "author_id": author.get("id", ""), "message": msg,
                "created_time": created, "like_count": c.get("like_count", 0),
                "is_negative": is_neg, "time_ago": created
            })
        return processed, None
    except Exception as e:
        logger.error(f"comments exception: {e}")
        return [], str(e)
...
def home():
    ...
    return render_template_string(HTML, token=tok, pages=pgs, posts=pst, sel=sel,
                                  post_id=None, comments=None, selected_post=None,
                                  alert=alert, alert_type=alert_type, total_likes=0,
                                  negative_count=0, debug_info=None)

@app.route("/login")
def login():
    state_val = f"betelgeuse_{int(datetime.now().timestamp())}"
    ...
    url = (f"https://www.facebook.com/v19.0/dialog/oauth?"
           ...
           f"&auth_type=rerequest&state={state_val}")
    logger.info(f"OAuth URL: {url[:120]}...")
    return redirect(url)
...
def cb():
    ...
    logger.info(f"Headers Host: {request.headers.get('Host', 'N/A')}")
    ...
    if error:
        logger.warning(f"OAuth denied: {error} | reason: {error_reason}")
        session["err"] = (f"<strong>Permission Required</strong><br>"
                          f"You declined access: <code>{error_reason}</code>.<br>"
                          f"To moderate comments, all three permissions are necessary.<br><br>"
                          ...
                          f"Click 'Login with Facebook' below to try again.")
...
def cm():
    ...
    return render_template_string(HTML, token=session.get("tok"),
                                  pages=pages(session.get("tok")), posts=pst, sel=pg,
                                  post_id=post_id, comments=comments, selected_post=selected_post,
                                  alert=None, alert_type="info", total_likes=total_likes,
                                  negative_count=negative_count, debug_info=None)
...
<style>body{font-family:Inter,system-ui;max-width:800px;margin:40px auto;padding:0 24px;line-height:1.7;color:#1c1e21}h1{color:#1877f2}h2{color:#333;margin-top:32px;font-size:18px}.badge{background:#e7f3ff;color:#1877f2;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:600}.footer{margin-top:40px;padding-top:20px;border-top:1px solid #ddd;color:#65676b;font-size:13px}a{color:#1877f2}</style></head><body>
<h1>Privacy Policy</h1><p><span class="badge">Last updated: May 30, 2026</span></p><p><strong>Betelgeuse Serviços de TI</strong> — CNPJ 51.770.524/0001-87</p>
<h2>1. Information We Process</h2><p>Our application processes the following data <strong>in real-time only</strong>:</p><ul><li>Facebook Page names and IDs you administer (via <code>pages_show_list</code>)</li><li>Post content, IDs, and creation dates (via <code>pages_read_engagement</code>)</li><li>Comment author names, author IDs, comment text, creation dates, and like counts (via <code>pages_read_user_content</code>)</li></ul>
<h2>2. No Data Storage</h2><p><strong>We do not store, persist, or retain any user data on our servers.</strong> All data is fetched directly from Facebook's Graph API and displayed in your browser session.</p>
<h2>3. Data Retention</h2><p>Data is retained only for the duration of your active session (typically less than 30 minutes).</p>
<h2>4. Data Sharing</h2><p>We do not share, sell, rent, or transfer any user data to third parties. We do not use comment data to train AI models.</p>
<h2>5. Your Rights</h2><p>You have the right to:</p><ul><li>Revoke app permissions at any time via <a href="https://www.facebook.com/settings?tab=applications" target="_blank">Facebook Settings → Apps</a></li><li>Request deletion of any cached session data via our <a href="/delete">Data Deletion</a> page</li><li>Contact us at <a href="mailto:falecom@mariomello.com.br">falecom@mariomello.com.br</a></li></ul>
<h2>6. Contact</h2><p>Betelgeuse Serviços de TI<br>CNPJ: 51.770.524/0001-87<br>Email: <a href="mailto:falecom@mariomello.com.br">falecom@mariomello.com.br</a><br>Address: Navegantes, SC, Brazil</p>
...
<h1>Terms of Service</h1><p><span class="badge">Effective: May 30, 2026</span></p><p><strong>Betelgeuse Serviços de TI</strong> — CNPJ 51.770.524/0001-87</p>
<h2>1. Service Description</h2><p>Betelgeuse TI Comment Moderation is a real-time tool that enables Facebook Page administrators to view and monitor public comments on posts they manage.</p>
<h2>2. Eligibility</h2><p>You must be at least 18 years old and an administrator of the Facebook Page you wish to monitor.</p>
<h2>3. Permitted Use</h2><p>You agree to use this service solely for:</p><ul><li>Monitoring and moderating comments on Facebook Pages you administer</li><li>Improving customer service response times</li></ul><p>You may NOT use this service to:</p><ul><li>Access Pages you do not administer</li><li>Scrape or bulk-download user data</li><li>Use comment data to train AI/ML models</li><li>Share user content with unauthorized third parties</li></ul>
<h2>4. Data Processing</h2><p>All data processing occurs in real-time. We do not store Facebook user data on our servers.</p>
<h2>5. Termination</h2><p>We may suspend access for violations of these terms or Facebook's Platform Policies. You may terminate use at any time by disconnecting the app via Facebook Settings.</p>
<h2>6. Governing Law</h2><p>These terms are governed by the laws of Brazil. Disputes shall be resolved in the courts of Navegantes, SC.</p>
<h2>7. Contact</h2><p>Email: <a href="mailto:falecom@mariomello.com.br">falecom@mariomello.com.br</a></p>
...
<style>body{font-family:Inter,system-ui;max-width:800px;margin:40px auto;padding:0 24px;line-height:1.7;color:#1c1e21}h1{color:#1877f2}h2{color:#333;margin-top:32px;font-size:18px}.badge{background:#ffebee;color:#c62828;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:600}.success{background:#e8f5e9;border:1px solid #a5d6a7;color:#2e7d32;padding:16px;border-radius:12px;margin:16px 0}.footer{margin-top:40px;padding-top:20px;border-top:1px solid #ddd;color:#65676b;font-size:13px}a{color:#1877f2}.btn{background:#1877f2;color:#fff;border:0;padding:12px 24px;border-radius:10px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-block}</style></head><body>
<h1>Data Deletion Request</h1><p><span class="badge">GDPR / LGPD Compliant</span></p><p><strong>Betelgeuse Serviços de TI</strong> — CNPJ 51.770.524/0001-87</p>
<h2>How to Delete Your Data</h2><div class="success"><strong>✅ Good news:</strong> Our application does not store any personal data on our servers. All Facebook data is processed in real-time and exists only during your active browser session.</div>
<h2>Immediate Steps (Instant)</h2><ol><li><strong>Revoke App Access:</strong> Go to <a href="https://www.facebook.com/settings?tab=applications" target="_blank">Facebook Settings → Apps & Websites</a>, find "Betelgeuse TI Comment Moderation", and click "Remove".</li><li><strong>Clear Session:</strong> Click <a href="/logout" class="btn" style="margin-left:8px">Logout from App</a> to clear your current session.</li></ol>
<h2>Contact Us for Confirmation</h2><p>If you would like written confirmation that no data is retained, contact us:</p><ul><li>Email: <a href="mailto:falecom@mariomello.com.br">falecom@mariomello.com.br</a></li><li>Subject line: <code>Data Deletion Request — [Your Facebook User ID or Email]</code></li><li>Response time: Within 48 hours (business days)</li></ul>
<h2>Legal Rights</h2><p>Under Brazil's LGPD and the EU's GDPR, you have the right to erasure. Since we do not store personal data, fulfillment is immediate upon app removal.</p>
...
<h1>Data Use Agreement</h1><p><span class="badge">Meta Platform Supplemental Terms</span></p><p><strong>Betelgeuse Serviços de TI</strong> — CNPJ 51.770.524/0001-87</p>
<h2>1. Purpose of Data Use</h2><p>We use Facebook Platform data solely for <strong>real-time comment moderation on Facebook Pages administered by the authenticated user</strong>.</p>
<h2>2. Data We Access</h2><table style="width:100%;border-collapse:collapse;margin:16px 0"><tr style="background:#f5f6f7"><th style="text-align:left;padding:12px;border:1px solid #ddd">Permission</th><th style="text-align:left;padding:12px;border:1px solid #ddd">Data Accessed</th><th style="text-align:left;padding:12px;border:1px solid #ddd">Use</th></tr><tr><td style="padding:12px;border:1px solid #ddd"><code>pages_show_list</code></td><td style="padding:12px;border:1px solid #ddd">Page name, ID, category</td><td style="padding:12px;border:1px solid #ddd">Display list of Pages user manages</td></tr><tr><td style="padding:12px;border:1px solid #ddd"><code>pages_read_engagement</code></td><td style="padding:12px;border:1px solid #ddd">Post ID, message, created_time, permalink</td><td style="padding:12px;border:1px solid #ddd">List posts for user to select</td></tr><tr><td style="padding:12px;border:1px solid #ddd"><code>pages_read_user_content</code></td><td style="padding:12px;border:1px solid #ddd">Comment author name, author ID, message, created_time, like_count</td><td style="padding:12px;border:1px solid #ddd">Display comments for moderation</td></tr></table>
<h2>3. Prohibited Uses</h2><p>We expressly commit to NOT:</p><ul><li>Store Facebook user data beyond the active session</li><li>Use data for advertising or marketing purposes</li><li>Sell, rent, or transfer data to third parties</li><li>Use comment content to train AI or ML models</li><li>Access Pages not administered by the authenticated user</li></ul>
<h2>4. Compliance</h2><p>We comply with Meta Platform Terms, Meta Data Processing Terms, Brazilian LGPD, and EU GDPR.</p>
<h2>5. Contact</h2><p>Email: <a href="mailto:falecom@mariomello.com.br">falecom@mariomello.com.br</a></p>
